...
public class kingdom_ministry_adapter extends SimpleCursorAdapter {

    private int layout;
    private class_functions funct;
    private SQLiteDatabase database;

    public kingdom_ministry_adapter(Context context,
                                    String[] from, int[] to,
        ...
        try {
            ...
            if (img.booleanValue()) {
                if (funct.ExternalStorageState()) {
                    File imgFile = new File(funct.get_dir_app() + "/img/kingdom_ministry/"
                            + name + ".jpg");
                    if (imgFile.exists()) {
